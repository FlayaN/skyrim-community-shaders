...
VariableRateShading::VariableRateShading(ComPtr<ID3D11Device> device)
{
	active = false;
	...
	if (!nvapiLoaded) {
		...
		if (result != NVAPI_OK) {
			logger::error("Failed to load NVAPI {}", (int)result);
			return;
		}
		nvapiLoaded = true;
	}
...
void VariableRateShading::UpdateTargetInformation(int a_targetWidth, int a_targetHeight, TextureMode a_mode, float leftProjX, float leftProjY, float rightProjX, float rightProjY)
{
	this->targetWidth = a_targetWidth;
	this->targetHeight = a_targetHeight;
	this->targetMode = a_mode;
	proj[0][0] = leftProjX;
	proj[0][1] = leftProjY;
	proj[1][0] = rightProjX;
	proj[1][1] = rightProjY;
...
void VariableRateShading::EndFrame()
{
	if (currentSingleEyeRT > 0) {
		if (currentSingleEyeRT != singleEyeOrder.size()) {
			logger::debug("Found {} single eye render targets in current frame", currentSingleEyeRT);
			...
			for (size_t i = singleEyeOrder.size(); i < currentSingleEyeRT; ++i) {
				singleEyeOrder.push_back('S');
			}
			logger::debug("Guessing order of render targets as {}", singleEyeOrder);
			if (!g_config.ffr.overrideSingleEyeOrder.empty()) {
				if (g_config.ffr.overrideSingleEyeOrder.size() == singleEyeOrder.size()) {
					...
					logger::debug("Overriding order with {}", singleEyeOrder);
				} else {
					logger::debug("Not using configured override since it does not match number of render targets: {}", g_config.ffr.overrideSingleEyeOrder);
				}
			}
		}
	}
...
void VariableRateShading::PostOMSetRenderTargets(UINT numViews, ID3D11RenderTargetView* const* renderTargetViews,
	ID3D11DepthStencilView*)
{
	...
	if (targetMode == TextureMode::SINGLE && ResolutionMatches(td.Width, 2 * targetWidth) && ResolutionMatches(td.Height, targetHeight)) {
		...
		if (currentSingleEyeRT < singleEyeOrder.size()) {
			...
			logger::debug("VRS: Single eye target, don't know which eye");
			DisableVRS();
		}
		++currentSingleEyeRT;
	} else {
...
void VariableRateShading::ApplyCombinedVRS(int width, int height)
{
	if (!active)
		return;
...
void VariableRateShading::ApplyArrayVRS(int width, int height)
{
	if (!active)
		return;
...
void VariableRateShading::ApplySingleEyeVRS(int eye, int width, int height)
{
	if (!active)
		return;
...
void VariableRateShading::SetupSingleEyeVRS(int eye, int width, int height, float projX, float projY)
{
	...
	logger::info("Creating VRS pattern texture for eye {}  of size {}x{}", eye, vrsWidth, vrsHeight);
	...
	if (FAILED(result)) {
		Shutdown();
		logger::error("Failed to create VRS pattern texture for eye {}: {:x}", eye, result);
		return;
	}
	...
	if (status != NVAPI_OK) {
		Shutdown();
		logger::error("Failed to create VRS pattern view for eye {}: {}", eye, (int)status);
		return;
	}
}
...
void VariableRateShading::SetupCombinedVRS(int width, int height, float leftProjX, float leftProjY, float rightProjX, float rightProjY)
{
	...
	logger::info("Creating combined VRS pattern texture of size {}x{} for input texture size {}x{}", vrsWidth, vrsHeight, width, height);
	...
	if (FAILED(result)) {
		Shutdown();
		logger::error("Failed to create combined VRS pattern texture: {:x}", result);
		return;
	}
	...
	if (status != NVAPI_OK) {
		Shutdown();
		logger::error("Failed to create combined VRS pattern view: {}", (int)status);
		return;
	}
}
...
void VariableRateShading::SetupArrayVRS(int width, int height, float leftProjX, float leftProjY, float rightProjX, float rightProjY)
{
	...
	logger::info("Creating array VRS pattern texture of size {}x{}", vrsWidth, vrsHeight);
	...
	if (FAILED(result)) {
		Shutdown();
		logger::error("Failed to create array VRS pattern texture: {:x}", result);
		return;
	}
	...
	logger::info("Creating array shading rate resource view");
	NV_D3D11_SHADING_RATE_RESOURCE_VIEW_DESC vd = {};
	vd.version = NV_D3D11_SHADING_RATE_RESOURCE_VIEW_DESC_VER;
	vd.Format = td.Format;
	...
	if (status != NVAPI_OK) {
		Shutdown();
		logger::error("Failed to create array VRS pattern view: {}", (int)status);
		return;
	}
}